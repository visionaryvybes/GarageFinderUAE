...
const NAV_LINKS = [
  ...
  { href: "/my-car", label: "My Car", icon: Car },
    ...
    <>
      <nav className="sticky top-0 z-50 w-full bg-black/95 backdrop-blur-xl border-b border-[#1a1a1a]">
        ...
        <div className="max-w-7xl mx-auto px-3 h-14 flex items-center gap-3">
            ...
            {NAV_LINKS.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition-all whitespace-nowrap shrink-0 ${
                    isActive
          ...
          {/* Mobile: bottom tab bar indicator + hamburger */}
          <div className="md:hidden ml-auto flex items-center gap-2">
            <span className="text-[11px] text-zinc-600 hidden sm:block">
              {NAV_LINKS.find(l => l.href === pathname || (l.href !== "/" && pathname.startsWith(l.href)))?.label || "GarageFinder"}
            </span>
            <button
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-4 border-b border-[#1a1a1a]">
                <Link href="/" onClick={() => setMobileOpen(false)} className="flex items-center gap-2">
                  <div className="w-7 h-7 bg-gradient-to-br from-zinc-800 to-zinc-950 border border-zinc-700 rounded-xl flex items-center justify-center">
                    <Wrench className="w-3.5 h-3.5 text-blue-400" />
                  </div>
                ...
                <button
                  onClick={() => setMobileOpen(false)}
                  className="p-1.5 rounded-lg bg-[#111] border border-[#1a1a1a] text-zinc-500"
                >
                  ...
                  return (
                    ...
                    >
                      ...
                      {"badge" in link && link.badge && (
                        <span className="text-[9px] font-black px-1.5 py-0.5 rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-400">
                          {link.badge}
                        </span>
                      )}
                    </Link>
